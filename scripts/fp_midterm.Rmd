...
pacman::p_load(tidyverse, dplyr, janitor, here, rio, gapr,glue)
```
...
final_plots <- district_differences %>% 
  mutate(plots = pmap(list(district_label,school_count_label,data), ~{
    ggplot(..3,aes(academic_year,achieve_diff))+
      geom_errorbar(aes(ymin = achieve_diff-v_se,
                    ymax=achieve_diff+v_se))+
      geom_hline(yintercept=0,
             color = "red")+
      geom_point(data = filter(..3,achieve_diff<=0),
             color = "#d35cff")+
      geom_point(data = filter(..3,achieve_diff>0),
             color = "#5ca3ff")+
      labs(title = ..1,
...
district_school_count <- assessment_agg %>% 
  group_by(district) %>% 
  summarize(school_count = min(n_schools)) #took the min value to account for school closures

dists <- unique(method2$district)[1:100] 

district_differences <- method2 %>% 
  filter(district %in% dists) %>% 
  left_join(district_school_count, by = "district") %>% 
  select(-auc)
